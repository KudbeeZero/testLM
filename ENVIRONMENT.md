...
service or key so we never lose track.
...
> Never put live keys in this document or in chat.
...
the `os-agent/` folder. It is excluded from Git via `.gitignore`.
...
### Provider switching
Set `MODEL_PROVIDER=local|gemini|grok|deepseek` in `.env` to choose which the
os-agent uses.
...
## 5. AWS defensive foundation (Phase 1)

Account `196856329692` · region `us-east-1` · auth via `aws login` (12h, renew 90d).

| Service | Resource | Destination | Status |
| --- | --- | --- | --- |
| CloudTrail | `kudbee-management` (multi-region, management events) | s3 `kudbee-cloudtrail-196856329692-us-east-1` | Logging |
| GuardDuty | Detector `ccd0002ffc4a782069d967a9845671a8` | — | ENABLED |
| VPC Flow Logs | `fl-02e88c213ffa065a9` (VPC `vpc-0027c7c93792e84e4`, ALL traffic) | s3 `kudbee-flowlogs-196856329692-us-east-1` | ACTIVE |
| CloudWatch alarms | `kudbee-ec2-cpu-high-1`, `kudbee-guardduty-findings` | — | Active |

**Retention:** 90-day S3 lifecycle on both buckets (`kudbee-cloudtrail-…`,
`kudbee-flowlogs-…`) — objects expire after 90 days, incomplete multipart uploads
abort after 90 days. No storage-class transitions. No versioning enabled.

**Cost notes:** CloudTrail management events (first 5 trails) free; S3 storage minimal.
GuardDuty free 30-day trial, then low per-event (~$0.10/1M). VPC Flow Logs to S3
= S3 storage only (no per-GB ingestion). Basic CloudWatch alarms free. No SNS
notification actions added (would incur cost).

**Not enabled (by design):** Bedrock (no model access), no SNS, no EventBridge
routing, no learning pipeline (Phase 3 deferred).
...
- [ ] DeepSeek V4 Flash integration (in progress)
...
and mirror memory to Upstash Redis.
